...
public class ExpenseMvcController {
    ...
    @GetMapping("/expenses/bar-graph")
    public String showExpenseBarGraph(Model model) {
        ...
        // Sort expensesByDate by date (optional)
        List<LocalDate> sortedDates = expensesByDate.keySet().stream()
                .sorted()
                .collect(Collectors.toList());

        List<Double> amounts = sortedDates.stream()
                .map(expensesByDate::get)
                .collect(Collectors.toList());
        ...
        // Return the view name for the bar graph
